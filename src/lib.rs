	...
	fn stress_test_various_sizes_with_random_chunk_loss() {
		...
		for data_size in data_sizes.iter() {
			...
			for &n_chunks in chunk_configs.iter() {
				...
				let mut rng = rand::rngs::SmallRng::seed_from_u64((*data_size as u64) ^ (n_chunks as u64));
				...
				println!("    Using {} chunks out of {} (threshold: {})", 
					available_chunks.len(), n_chunks, threshold);
				
				// Recover data from available chunks
				let reconstructed = reconstruct(
					n_chunks,
					available_chunks.into_iter(),
					original_data.len()
				).unwrap();
